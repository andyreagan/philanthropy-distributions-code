...
for i = 1:3
    ...
    if i==1
        ...
        c=62000; % biggest gift
        N=200; % number of gifts following power law
        N2=542; % total number of gifts
        alpha=0.44; % corresponding to
        ...
        %% add a fit to the reference distribution
        minfit = 5; %110
        maxfit = 200; %1800
        [alpha,gamma]=plotpowerfit(minfit:maxfit,gifts(minfit:maxfit)','Offset',offsets(1),'gammaconfrep',2,'prep',2);
        ...
        gifts = [.6.*gifts .25.*gifts .1.*gifts .05.*gifts];
        ...
        % this is gamma on the shoulder
        minfit = 200; %110
        maxfit = 700; %1800
        [alpha,gamma]=plotpowerfit(minfit:maxfit,gifts(minfit:maxfit)','Offset',offsets(2),'gammaconfrep',2,'prep',2);
        
        clear tmptxt
        % for having the comparison label
        %         tmptxt{1} ='Comparison';
        %         tmptxt{2} =sprintf('   \\alpha = %1.2f',alpha);
        %         tmptxt{3} =sprintf('   \\gamma = %1.2f',gamma);
        tmptxt{1} =sprintf('\\alpha = %1.2f',alpha);
        tmptxt{2} =sprintf('\\gamma = %1.2f',gamma);
        text(.41,.39,tmptxt,...
            'fontsize',powerlabelsize,...
    ...
    if i==2
        ...
        %% plot the areas
        X=log10([donors(1:40),fliplr(donors(1:40))]);                %create continuous x value array for plotting
        Y=log10([gifts(1:40),fliplr(donations(1:40)')]);   %create y values for out and then back
        fill(X,Y,[153 204 255]/255);
        hold on;
        a = annotation('textarrow', [.435 .412], [.77 .735]-.18, 'String' , 'Missing gifts','fontsize',pointersize,'Color',0.01*[1 1 1],'HeadWidth',5,'HeadLength',5);
        %X(1) and Y(1) specify the position of the tail end of the arrow
        ...
        for j=[1 2 3]
            ...
            plotlogspace =1;
            if plotlogspace
                pts2plot=floor(logspace(0,log10(length(donations)),75));
            else
                pts2plot=1:length(donations);
            end
            
            tmph(j) = plot(log10(pts2plot),log10(donations(pts2plot)),'Marker',tmpsym{j},'MarkerFaceColor',tmpcol{j},'LineStyle','none');
        ...
        % this is gamma on the shoulder
        fitregion=110:1800;
        [alpha,gamma]=plotpowerfit(fitregion,donations(fitregion),'Offset',offsets(3),'gammaconfrep',2,'prep',2);
    ...
    if i==3
        ...
        fitlow=8;
        fithigh=200;
        [alpha,gamma]=plotpowerfit(fitlow:fithigh,donations(fitlow:fithigh),'Offset',offsets(4),'gammaconfrep',2,'prep',2);
        
        clear tmptxt
        
        tmptxt{1} =sprintf('\\alpha = %1.2f',alpha);
        tmptxt{2} =sprintf('\\gamma = %1.2f',gamma);
        csvwrite('relig_inst_corrected_gamma.csv',gamma);
        text(.65,.53,tmptxt,...
            'fontsize',powerlabelsize,...
            'fontname','helvetica',...
            'units','normalized',...
        ...
        tmpXcoord = 0.24;
        tmpYcoord = 0.08;
        tmpstr = 'Religious Inst, Corrected';
        text(tmpXcoord,tmpYcoord,tmpstr,...
            'fontsize',6,...
            'fontname','helvetica',...
            'units','normalized'... % ,...
